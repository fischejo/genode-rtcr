...
namespace Init_rtcr { struct Child_registry; }


class Init_rtcr::Child_registry : public Name_registry, Child_list
{
	private:
		...
		bool _unique(const char *name) const
		{
			...
			Genode::List_element<Init_rtcr::Child> const *curr = first();

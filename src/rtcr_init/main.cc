...
namespace Init_rtcr { struct Main; }


struct Init_rtcr::Main : State_reporter::Producer, Child::Default_route_accessor,
                    Child::Default_caps_accessor, Child::Ram_limit_accessor
{
	Env &_env;

	Registry<Init_rtcr::Parent_service> _parent_services { };
	...
	Rtcr::Init_module *_rtcr_module;
	Init_rtcr::Root_component *_rtcr_root;
	...
	Main(Env &env) : _env(env)
	{
		...
		/* create rtcr module */
		Module_name module_name = _config.xml().sub_node("module")
			.attribute_value("name", Module_name());
		_rtcr_module = Module_factory::get(module_name)->create(_env, _heap);
		_rtcr_root = new(_heap) Init_rtcr::Root_component(_env, _heap, *_rtcr_module);
...
void Init_rtcr::Main::_update_parent_services_from_config()
{
	...
	node.for_each_sub_node("service", [&] (Xml_node service) {
		...
		if (!registered) {
			new (_heap) Init_rtcr::Parent_service(_parent_services, _env, name);
			if (_verbose->enabled()) {
				if (first_log)
					log("parent provides");
				log("  service \"", name, "\"");
...
void Init_rtcr::Main::_destroy_abandoned_parent_services()
{
	_parent_services.for_each([&] (Parent_service &service) {
...
void Init_rtcr::Main::_update_aliases_from_config()
{
	/* remove all known aliases */
	while (_children.any_alias()) {
		Init_rtcr::Alias *alias = _children.any_alias();
...
void Init_rtcr::Main::_abandon_obsolete_children()
{
...
void Init_rtcr::Main::_update_children_config()
{
	for (;;) {
...
void Init_rtcr::Main::_handle_config()
{
	_config.update();
	...
	try {
		_config_xml.for_each_sub_node("start", [&] (Xml_node start_node) {
			...
			try {
				Init_rtcr::Child &child = *new (_heap)
					Init_rtcr::Child(_env, _heap, *_verbose,
					            Init_rtcr::Child::Id { ++_child_cnt }, _state_reporter,
	...
	catch (Init_rtcr::Child_registry::Alias_name_is_not_unique) { }

	/*
	 * Init_rtcriate RAM sessions of all new children
	 */
	_children.for_each_child([&] (Child &child) {
		child.initiate_env_ram_session(); });

	/*
	 * Init_rtcriate remaining environment sessions of all new children
	 */
	_children.for_each_child([&] (Child &child) {
...
void Libc::Component::construct(Libc::Env &env) { static Init_rtcr::Main main(env); }

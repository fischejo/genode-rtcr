...
Init_rtcr::Child::Apply_config_result
Init_rtcr::Child::apply_config(Xml_node start_node)
{
...
static Init_rtcr::Ram_quota assigned_ram_from_start_node(Genode::Xml_node start_node)
{
	using namespace Init_rtcr;
...
void Init_rtcr::Child::apply_ram_upgrade()
{
...
void Init_rtcr::Child::apply_ram_downgrade()
{
	Ram_quota const assigned_ram_quota = assigned_ram_from_start_node(_start_node->xml());
...
void Init_rtcr::Child::report_state(Xml_generator &xml, Report_detail const &detail) const
{
	xml.node("child", [&] () {
...
void Init_rtcr::Child::init(Pd_session &session, Pd_session_capability cap)
{
...
void Init_rtcr::Child::init(Cpu_session &session, Cpu_session_capability cap)
{
...
Init_rtcr::Child::Route Init_rtcr::Child::resolve_session_request(Service::Name const &service_name,
	...
	try {
		...
		for (; ; service_node = service_node.next()) {
			...
			for (; ; target = target.next()) {
				...
				if (target.has_type("rtcr")) {

					try {
						return Route { find_service(_rtcr_services, service_name, no_filter),
...
void Init_rtcr::Child::filter_session_args(Service::Name const &service,
...
Genode::Affinity Init_rtcr::Child::filter_session_affinity(Affinity const &session_affinity)
{
...
void Init_rtcr::Child::announce_service(Service::Name const &service_name)
{
	if (_verbose.enabled())
...
void Init_rtcr::Child::resource_request(Parent::Resource_args const &args)
{
...
Init_rtcr::Child::Child(Env                      &env,
		   ...
		   Registry<Genode::Service> &rtcr_services)
:

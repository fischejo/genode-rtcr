...
Pd_session_component &Pd_session_handler::session()
{
    return _session;
}


Genode::Local_service &Pd_session_handler::service()
{
    return _service;
}


Pd_root &Pd_session_handler::root()
{
    return _root;
}


Genode::List<Kcap_badge_info> &Pd_session_handler::kcap_mappings()
{
    return _kcap_mappings;
}


Pd_session_component &Pd_session_handler::_find_session(const char *label, Pd_root &pd_root)
{
...
// 4. includes
// 6. _create_kcap_mappings marking implm.


Genode::List<Kcap_badge_info> Pd_session_handler::_create_kcap_mappings(&Target_state state)
{
    using Genode::log;
    using Genode::Hex;
    using Genode::addr_t;
...
Genode::List<Ref_badge_info> Pd_session_handler::_mark_and_attach_designated_dataspaces(Attached_region_info &ar_info)
{
#ifdef DEBUG
    Genode::log("Ckpt::\033[33m", __func__, "\033[0m(...)");
#endif
...
void Pd_session_handler::_detach_and_unmark_designated_dataspaces(Genode::List<Ref_badge_info> &badge_infos,
...
void Pd_session_handler::prepare_checkpoint(Target_state &state)
{
#ifdef VERBOSE
    ...
    _kcap_mappings = _create_kcap_mappings(state);
...
void Pd_session_handler::checkpoint(Target_state &state)
{
#ifdef VERBOSE
    Genode::log("CPU_session_handler::checkpoint()");
#endif
  
    _prepare_pd_sessions(state);
}


void Pd_session_handler::_prepare_pd_sessions(Target_state &state);
{
    Genode::List<Stored_pd_session_info> &stored_infos = state._stored_pd_sessions;
    Genode::List<Pd_session_component> &child_infos = _root.session_infos();
...
// _state_alloc ?
void Pd_session_handler::_destroy_stored_pd_session(Target_state &state,
...
void Pd_session_handler::_prepare_native_caps(Target_state &state, Genode::List<Native_capability_info> &child_infos)
{
#ifdef VERBOSE
    Genode::log("Ckpt::\033[33m", __func__, "\033[0m(...)");
#endif

    Genode::List<Stored_native_capability_info> &stored_infos = state.stored_native_cap_infos;
...
void Pd_session_handler::_destroy_stored_native_cap(Target_state &state, Stored_native_capability_info &stored_info)
{
#ifdef VERBOSE
    Genode::log("Ckpt::\033[33m", __func__, "\033[0m(...)");
#endif
...
void Pd_session_handler::_prepare_signal_sources(Target_state &state, Genode::List<Signal_source_info> &child_infos)
{
#ifdef
    Genode::log("Ckpt::\033[33m", __func__, "\033[0m(...)");
#endif

    Genode::List<Stored_signal_source_info> &stored_infos = state.stored_source_infos;
...
void Pd_session_handler::_destroy_stored_signal_source(Target_state &state,
...
void Pd_session_handler::_prepare_signal_contexts(Target_state &state,
						  Genode::List<Signal_context_info> &child_infos)
{
#ifdef VERBOSE
    Genode::log("Ckpt::\033[33m", __func__, "\033[0m(...)");
#endif
    Genode::List<Stored_signal_context_info> &stored_infos = state.stored_context_infos;
...
void Pd_session_handler::_destroy_stored_signal_context(Target_state &state,
							Stored_signal_context_info &stored_info)s
{
#ifdef VERBOSE
    Genode::log("Ckpt::\033[33m", __func__, "\033[0m(...)");
#endif
...
void Pd_session_handler::_destroy_list(Genode::List<Kcap_badge_info> &list)
{

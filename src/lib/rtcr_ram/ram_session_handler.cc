...
Ram_session_handler::Ram_session_handler(Genode::Env &env,
    ...
    _md_alloc(md_alloc),
    _ep(ep),
    _root(env, md_alloc, ep, granularity, bootstrap),
    _service("RAM", _root),
...
Ram_session_component &Ram_session_handler::_find_session(const char *label, Ram_root &ram_root)
{
#ifdef DEBUG
...
void Ram_session_handler::prepare_checkpoint(Target_state &state)
{
#ifdef VERBOSE
    Genode::log("Ckpt::\033[33m", __func__, "\033[0m()");
#endif
...
void Ram_session_handler::checkpoint(Target_state &state)
{
#ifdef VERBOSE
    Genode::log("Ckpt::\033[33m", __func__, "\033[0m()");
#endif
    _prepare_ram_sessions(state, _root->session_infos());
}


void Ram_session_handler::_prepare_ram_sessions(Target_state &state, Genode::List<Ram_session_component> &child_infos)
{
#ifdef VERBOSE
    Genode::log("Ckpt::\033[33m", __func__, "\033[0m()");
#endif    
...
void Ram_session_handler::_destroy_stored_ram_session(Target_state &state,
...
void Ram_session_handler::_prepare_ram_dataspaces(Target_state &state,
...
Stored_ram_dataspace_info &Ram_session_handler::_create_stored_ram_dataspace(Target_state &state,
...
void Ram_session_handler::_destroy_stored_ram_dataspace(Target_state &state,
...
void Ram_session_handler::_checkpoint_dataspaces(Target_state &state)
{
#ifdef VERBOSE
    Genode::log("Ckpt::\033[33m", __func__, "\033[0m()");
#endif    

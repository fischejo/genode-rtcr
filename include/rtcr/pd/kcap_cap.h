 ...
 */
struct Rtcr::Kcap_cap : Genode::List<Kcap_cap>::Element
{
	Genode::addr_t            kcap;
	Genode::Native_capability cap;
	Genode::String<32>        const label;

	Kcap_cap(Genode::addr_t kcap,
				  Genode::Native_capability cap,
				  const char* label)
		: kcap(kcap), cap(cap), label(label) { }

	Kcap_cap *find_by_kcap(Genode::addr_t kcap) {
		if(kcap == this->kcap)
			return this;
		Kcap_cap *info = next();
	...
	Kcap_cap *find_by_badge(Genode::uint16_t badge) {
